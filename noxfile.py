...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/sap_car/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/sap_car/*")
            ...
            #    "--include=src/saltext/sap_car/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/sap_car/config/schemas",
    )
